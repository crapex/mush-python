...
class CmdWalkDirection(MudCommand): 
    ...
    2020-3-28: add timeout (default 5s) for CmdWalkDirection.
    '''
    _initTriList = (
            TriggerDefinition('dir_ok', r'^[> ]*(\S.*)\s*-\s*(?:\[(野外|城内|门派|0)\])?(?:\s*\[(存盘点|玩家储物柜)\])?$', '_onSuccess', 1),
            TriggerDefinition('dir_fail', r'^[> ]*哎哟，你一头撞在墙上，才发现这个方向没有出路。|^[> ]*你逃跑失败', '_onFail', 1),
    ...
    def __init__(self, owner, group):
        ...
        self._timerout = Timer(self, group, self._TIME_OUT_, self._onTimeout, name = "timeout")           
    ...
    def _beforeExecute(self, **params):
        ...
        self._timerout.Reset()
        
    def _afterExecute(self, state):
        self._timerout.Enabled = False
        super()._afterExecute(state)